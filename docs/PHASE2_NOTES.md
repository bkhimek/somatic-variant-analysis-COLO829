...
**Date:** 2026-08-30
**Status:** **PHASE 2 SIGNED OFF 2026-08-30.** The `dev`-profile (driver-gene-BED-restricted) run succeeded end-to-end first try — all of `GET_PILEUP_SUMMARIES`, `CALCULATE_CONTAMINATION`, `MUTECT2`, `LEARN_READ_ORIENTATION_MODEL`, and `FILTER_MUTECT_CALLS` completed without error against the real GATK resource VCFs, confirming the DAG, containers, and every GATK command line are wired correctly. The `full`-profile (unrestricted, genome-wide) rerun surfaced two real, fixed bugs (`GetPileupSummaries`'s mandatory `-L`, a JVM-heap-in-Docker sizing issue) and then a third finding that isn't a bug at all: real research confirmed unsharded genome-wide Mutect2 is not how this tool is meant to be run by anyone, at any memory size available on this machine — see "Third `full`-profile attempt" and "Phase 2 sign-off" below for the full reasoning and the decision, made with the user. Getting here took the same "review isn't the same as execution" pattern as every phase so far — two real GATK-behaviour/environment bugs found only by executing the code, documented in order below. Originally built following the same GATK Best Practices tumour-normal design the repo's existing scaffolding already signposted (`panel_of_normals`, `germline_resource`, `common_biallelic_sites` params, and the `qc.contamination` warn/fail thresholds in `conf/resources.config`, were all already declared in Phase 0/1, unused until now).
...
## Third `full`-profile attempt — genuine architectural ceiling, not a heap-tuning bug (2026-08-30)

The `-Xmx6g` fix landed and was re-run. `MUTECT2` got much further this time — `Runtime.totalMemory()=6442450944` in the crash log confirms the JVM *did* get the full 6GB heap requested (unlike the previous attempt, where it silently got 2GB). It traversed essentially the entire genome (progress meter ran chr1 through chrY and the decoy contigs), ran for 6.40 minutes, logged "Shutting down engine" and "done" — and *then* still threw `java.lang.OutOfMemoryError: Java heap space`, in a call path building a second reference-file handle (`Mutect2Engine.makeStandardMutect2PostFilterReadTransformer` → `CachingIndexedFastaSequenceFile`). So this is not a repeat of the container-detection problem the last fix addressed — the JVM got exactly what it was given and it genuinely wasn't enough.

Before reflexively bumping `-Xmx` again, this was researched properly (same standard applied throughout this project to memory claims — see the bwa-mem2 correction in `PHASE1_NOTES.md` — real citations, not a guessed number), and the finding changes the diagnosis:

1. **GATK's own FAQ says not to do this.** The [Mutect2 FAQ](https://gatk.broadinstitute.org/hc/en-us/articles/360050722212-FAQ-for-Mutect2) states ~4GB is fine for exome, but for WGS the documented approach is to **scatter by interval** (`-L`) across many parallel Mutect2 shards and merge the results — not to run one unsharded JVM over the whole genome. We're doing the latter.
2. **Every production pipeline that implements this exact workflow scatters.** Broad's own [`gatk4-somatic-snvs-indels` WDL](https://github.com/gatk-workflows/gatk4-somatic-snvs-indels/blob/master/mutect2.wdl) defaults each scattered shard to ~3GB heap (`scatter_count=50`, i.e. 50 parallel Mutect2 processes, each covering ~1/50th of the genome). nf-core/sarek — the standard reference Nextflow implementation of this same PON+germline-resource design — sizes its `GATK4_MUTECT2` process at ~12GB container memory (~9.8GB heap) **per interval shard**, retrying at ~24GB. Neither ever runs a single unsharded genome-wide Mutect2 process.
3. **Unsharded genome-wide runs are a documented failure mode even at very high memory.** [GATK issue #5900](https://github.com/broadinstitute/gatk/issues/5900) reports an OOM even at `-Xmx300g`. A 2024 GATK community post has a Broad engineer (Louis Bergelson) recommending "`-Xmx28` in a 32GB container" as a *minimum* starting point for large-genome single-shot runs, with reports of needing 32–96GB heap, and a Biostars thread citing 100GB RAM taking 4.35 hours for chromosome 1 alone in a single-JVM run.

**Conclusion: this isn't a tunable heap-size bug, it's the wrong architecture for local execution.** Even the low end of the evidence above (`-Xmx28g`+ in a 32GB+ container) exceeds this machine's entire 26GB WSL2 ceiling (`PHASE1_NOTES.md`'s `.wslconfig` fix) — there is no `-Xmx` value between "OOMs" and "exceeds total available memory" to try next. This is the same kind of hard local-resource ceiling already hit once before in this project (bwa-mem2 index building genuinely needing ~30-110GB, solved via a one-off AWS burst rather than further local tuning) — not a mistake to fix, a real limit to route around.

**Also worth noting:** the current `full`-profile BAMs are still the same 10,000-read-pair genome-wide subsample from Phase 1, not real full-depth coverage. Running unsharded Mutect2 genome-wide is expensive largely from traversing and querying the germline-resource/PoN annotation resources across every part of the genome and its decoy contigs — regardless of how sparse the actual read data is — so this ceiling would be hit on this machine even before real sequencing depth becomes a factor.

**Decision presented to Krzysztof** — three options: (a) sign off Phase 2 on the already-successful `dev`-profile result and defer real full-genome Mutect2 execution to Phase 3+; (b) build real interval-scatter/gather into Module 4 now, ahead of when it's actually needed; (c) a middle-ground larger-but-still-local interval restriction (e.g. a full chromosome). **Decided: option (a).** See "Phase 2 sign-off" below.

---

## Phase 2 sign-off (decided with the user 2026-08-30)

Mirroring Phase 1's sign-off pattern: Phase 2's actual goal was to prove Modules 3–4 are built correctly — the right GATK tools, wired together in the right order, with the right inputs/outputs, running successfully against the real reference and the real GATK resource bundles. The `dev`-profile run already proves exactly that: `GET_PILEUP_SUMMARIES` → `CALCULATE_CONTAMINATION` → `MUTECT2` → `LEARN_READ_ORIENTATION_MODEL` → `FILTER_MUTECT_CALLS` all ran to completion, took real inputs and produced real (if variant-free, per the "Interval BED" caveat above) outputs in the expected shapes. That's the code-correctness bar this phase was meant to clear.

What the `dev`-profile run does **not** prove — and what unsharded genome-wide Mutect2 execution on this machine turned out to be structurally incapable of proving, not just currently misconfigured — is what real somatic calling looks like at real depth across a real genome. That was never actually in scope for a subsample smoke test anyway (echoing Phase 1's duplication-rate discussion: some questions need real production-scale execution to answer, and pretending a smoke test can answer them just produces a false sense of validation). **Phase 2 is being formally closed out on the `dev`-profile evidence, with two things explicitly carried forward rather than treated as Phase 2 blockers:**

1. **Real interval-scatter/gather architecture for Module 4** (splitting the genome into N chunks, running N parallel Mutect2 shards, merging VCFs/stats — matching nf-core/sarek's design) is needed regardless of memory, per GATK's own guidance, and will be built when Phase 3's real full-depth benchmarking data makes it actually necessary — not now, against 10,000-read subsample data where there's nothing real to scatter over.
2. **Compute for that real run** will very likely need to be another one-off cloud burst, the same pattern already used once for the bwa-mem2 index build (`PHASE1_NOTES.md`) — the research above put the realistic floor at ~28GB heap in a 32GB+ container just to avoid OOMing on an unsharded run, and proper scattering trades memory-per-shard for many more concurrent processes, which has its own resource footprint at real WGS scale.

Proceeding to Phase 3 (benchmarking against the NYGC COLO829 truth set) on this basis — which is also where the "is `filtered.vcf.gz` actually finding real variants" question gets a real answer, once real coverage data is in the loop.
