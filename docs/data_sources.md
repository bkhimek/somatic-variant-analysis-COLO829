...
- **Data:** https://www.nygenome.org/bioinformatics/3-cancer-cell-lines-on-2-sequencers/ — download the **COLO-829 (HiSeqX)** SNV/indel VCF specifically, not the NovaSeq one: HiSeqX is a platform match to the PRJEB27698 FASTQs (also HiSeq X Ten), which at least removes one axis of difference even though sequencing centre and library prep still differ. Exact file URLs weren't extractable through this session's web tools (the page's download links didn't resolve through automated fetching) — grab them directly from the page in your browser and record them below.
- **Access:** Open, no login
- **Genome build:** GRCh38 (confirm exact decoy/patch level once downloaded — see §2 below, must match what Mutect2/PoN/gnomAD are built against)
...
- **Exact download URL used:** TODO — fill in once you've pulled it from the NYGC page
- **High-confidence BED:** TODO — check whether NYGC publishes a callable-regions BED alongside the VCF, or whether the whole genome is treated as callable (record whichever is true)

**Caveat, carried into `docs/benchmarking_results.md`:** this truth set was generated by NYGC's own pipeline from a separate HiSeqX sequencing run/library prep, not the exact PRJEB27698 FASTQs this pipeline aligns. It is not an independent multi-platform consensus (unlike Craig et al. 2016, below) — it's one lab's own somatic calls, so treat concordance against it as "how well does Mutect2 agree with another GATK-family somatic pipeline on a related-but-not-identical COLO829 prep," not as an absolute ground truth. This is the exact caveat the plan's Phase 0 checklist asked to be documented, not quietly assumed.
