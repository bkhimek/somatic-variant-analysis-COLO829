...
**Date:** 2026-08-30
**Status:** Built and wired in this session. **Not yet run.** Same "review isn't the same as execution" caveat as every phase so far applies — expect at least one round of real bugs once this actually executes.
...
3. **`htslib` container tag for `PREPARE_TRUTH_VCF`** (`quay.io/biocontainers/htslib:1.21--h566b1c6_0`) is a reasonable, standard choice (matches the htslib version implied by the samtools 1.21 image already confirmed working in Phase 1) but hasn't itself been pulled/confirmed yet either.
