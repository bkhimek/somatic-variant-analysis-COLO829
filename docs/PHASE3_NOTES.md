...
**Date:** 2026-08-30
**Status:** First run found two real issues: a wrong container tag (fixed, see "First run — findings" below) and, much more significantly, the planned COLO-829 HiSeqX truth-set VCF turned out to be corrupted at the source in NYGC's own published archive, with no fix available anywhere (see "First run — findings" further down for the full forensic trail) — the truth set was switched to the COLO-829 NovaSeq VCF instead (same archive, confirmed intact). Not yet re-run against the corrected truth-set path. Same "review isn't the same as execution" caveat as every phase so far applies.
