...
**UPDATE 2026-08-30 — that "confirmed" file turned out to be corrupted, and the "140MB (stale)" note above was actually a real symptom, not stale metadata.** This section's "confirmed" check only verified the byte count, not the actual VCF content. Phase 3's first benchmarking run found the real content isn't COLO-829 HiSeqX data at all — see `docs/data_sources.md` §3's 2026-08-30 update for the full forensic trail (byte-level analysis, SHA-256-confirmed not a download issue, and independent corroboration that the whole repo/archive is short by almost exactly half). **The truth set actually in use going forward is the COLO-829 NovaSeq VCF from the same archive (confirmed intact), not the HiSeqX one this section originally reported.** This is exactly the kind of thing worth remembering for future phases: a byte-count match is not the same as a content check, and "the archive downloaded correctly" and "the file inside it is what it claims to be" are two different, both-necessary things to verify.
